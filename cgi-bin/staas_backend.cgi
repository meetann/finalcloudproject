...
conn=mysql.connect(user='root', password='q', host='localhost', database='cloud')
...
var5=commands.getoutput('sudo chmod 777 /share_info/'+st_name)
print var4
print var5
commands.getoutput('sudo echo /dev/share/'+st_name+'   /share_info/'+st_name+'     xfs    defaults     0 0 >> /etc/fstab ')
var1=commands.getstatusoutput('sudo echo /share_info/'+st_name+' 192.168.43.222\(rw,all_squash\) > /etc/exports')
...
fi.write('commands.getoutput("mount 192.168.43.14:/share_info/'+st_name+' /mnt/'+st_name+'")\n')
fi.write('commands.getoutput("chmod 777 /mnt/'+st_name+'")\n')
fi.write('commands.getoutput("mkdir /mnt/'+st_name+'/newdir")\n')
fi.close()
